      ...
      nodes {
        ...
        visits(first: 5) {
          nodes {
            ...
            endAt
            status
            assignedTeamMembers: team { nodes { id firstName lastName } }
          }
        }
        instructions
      }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Build a map from jobber_id to local uuid for linking
    ...
    for (const c of clients) {
      ...
      const secondaryPhone = c.secondaryPhones?.[1]?.number || null;
    ...
    for (const j of jobs) {
      ...
      // Get the latest visit for scheduling info
      const latestVisit = j.visits?.nodes?.[0];

      const assignedNames = latestVisit?.assignedTeamMembers?.nodes?.map(
        (m: any) => `${m.firstName || ""} ${m.lastName || ""}`.trim()
      ) || [];

      const assignedIds = latestVisit?.assignedTeamMembers?.nodes?.map(
        (m: any) => m.id
      ) || [];
