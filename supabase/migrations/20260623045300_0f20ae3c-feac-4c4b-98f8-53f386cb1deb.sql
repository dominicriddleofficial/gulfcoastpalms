...
DECLARE
  ...
  _claimed_pc uuid;
BEGIN
  -- STEP 1: link each jobber_client to a platform_customers row.
  -- Greedy row-by-row so each pc is claimed at most once.
  FOR _jc IN
  ...
  LOOP
    ...
    -- Find the oldest still-unclaimed platform_customers row that matches by
    -- normalized phone (last 10 digits) or email.
  ...
  -- STEP 2: insert/link platform_properties (unchanged)
  WITH ins AS (
  ...
  -- STEP 5: one visit per backfilled job that has none
  WITH insv AS (
