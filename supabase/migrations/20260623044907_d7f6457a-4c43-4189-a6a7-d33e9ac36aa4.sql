...
BEGIN
  ...
  -- jobber_client via normalized phone (last 10 digits) or email.
  -- Placeholder names ('', 'na', 'n/a') are NOT used as a sole match key.
  WITH candidates AS (
    SELECT jc.id AS jc_id, jc.jobber_id, jc.business_id,
  ...
  matched AS (
    ...
    JOIN public.platform_customers pc
     ...
     AND (
       (length(c.np) = 10 AND
          right(regexp_replace(coalesce(pc.phone,''),         '\D','','g'), 10) = c.np)
       OR
       (length(c.np) = 10 AND
          right(regexp_replace(coalesce(pc.secondary_phone,''),'\D','','g'), 10) = c.np)
       OR
       (length(c.np2) = 10 AND
          right(regexp_replace(coalesce(pc.phone,''),         '\D','','g'), 10) = c.np2)
       OR
       (c.lemail IS NOT NULL AND lower(pc.email) = c.lemail)
     )
     AND (pc.source_record_id IS NULL
  ...
  -- STEP 1c: insert net-new platform_customers for jobber_clients still unmatched.
  WITH ins AS (
  ...
  -- STEP 2: insert/link platform_properties from jobber_properties (unchanged)
  WITH ins AS (
  ...
  -- STEP 3: insert new platform_jobs with customer + property already linked (unchanged)
  WITH ins AS (
  ...
  -- STEP 5: one visit per backfilled job that has none (unchanged)
  WITH insv AS (
