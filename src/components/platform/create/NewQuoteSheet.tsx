...
export default function NewQuoteSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const save = async () => {
    ...
    const { data: numData } = await supabase.rpc("generate_next_number", {
      _business_id: selectedBusinessId, _record_type: "quote",
    });
    const quoteNumber = (typeof numData === "string" ? numData : `Q-${Date.now().toString().slice(-6)}`);
    ...
    if (error || !quote) { toast.error(error?.message || "Could not create quote"); setSaving(false); return; }
    ...
    if (lineRows.length) await supabase.from("platform_quote_line_items").insert(lineRows);
    ...
    <>
      <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
        <SheetContent side="bottom" className="h-[92vh] sm:h-auto sm:max-w-xl sm:right-0 sm:left-auto sm:inset-y-0 sm:rounded-l-xl overflow-y-auto">
          <SheetHeader><SheetTitle>New quote</SheetTitle></SheetHeader>
          <div className="space-y-3 mt-4">
            ...
            <div>
              ...
              <LineItemsEditor items={items} onChange={setItems} hideAmounts={!isOwner} />
            </div>
            {isOwner && (
              <>
          ...
          <div className="flex gap-2 mt-5 sticky bottom-0 bg-background py-3">
            ...
            <Button className="flex-1" onClick={save} disabled={saving}>{saving ? "Saving..." : "Create quote"}</Button>
          </div>
        </SheetContent>
      </Sheet>
