...
// Wrap existing lead dashboard in admin layout
...
import {
  Search, Phone, MessageSquare, CalendarIcon, Download,
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="font-body text-sm text-muted-foreground">{stats.total} total leads</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {[
            ...
            <Card key={label}><CardContent className="p-3 flex items-center gap-2">
              ...
              <div><p className="font-body text-xs text-muted-foreground">{label}</p><p className={cn("font-body text-lg font-bold", color)}>{value}</p></div>
            </CardContent></Card>
          ))}
        </div>
        ...
        <div className="space-y-3">
          {filtered.length === 0 && <p className="text-center text-muted-foreground font-body py-12">No leads found.</p>}
          {filtered.map((lead) => {
            const urgency = getUrgency(lead);
            const isExpanded = expandedId === lead.id;
            return (
              <Card key={lead.id} className={cn(urgency === "critical" && "border-red-400 bg-red-50/50", urgency === "warning" && "border-yellow-400 bg-yellow-50/50")}>
                  ...
                  <div className="flex items-center justify-between gap-2 cursor-pointer" onClick={() => setExpandedId(isExpanded ? null : lead.id)}>
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0">
                        <p className="font-body font-semibold truncate">{lead.name}</p>
                        <p className="font-body text-xs text-muted-foreground">{format(new Date(lead.created_at), "MMM d, h:mm a")}{lead.service && ` · ${lead.service}`}</p>
                      </div>
                    </div>
                  ...
                  {isExpanded && (
                    <div className="mt-4 pt-4 border-t border-border space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm font-body">
                        {lead.phone && <div className="flex items-center gap-2"><Phone className="w-4 h-4 text-muted-foreground" /><a href={`tel:${lead.phone}`} className="text-primary hover:underline">{lead.phone}</a><a href={`sms:${lead.phone}`} className="text-muted-foreground hover:text-primary"><MessageSquare className="w-4 h-4" /></a></div>}
                      ...
                      {lead.message && <div><p className="font-body text-xs text-muted-foreground mb-1">Message</p><p className="font-body text-sm bg-secondary p-3 rounded-lg">{lead.message}</p></div>}
                      <div><p className="font-body text-xs text-muted-foreground mb-2">Status</p>
                        <div className="flex flex-wrap gap-2">{STATUSES.map((s) => <Button key={s} variant={lead.status === s ? "default" : "outline"} size="sm" className="font-body text-xs capitalize" onClick={() => updateLead(lead.id, { status: s })}>{s}</Button>)}</div>
                      </div>
                      <div><p className="font-body text-xs text-muted-foreground mb-2">Notes</p>
                        <Textarea className="font-body text-sm" rows={3} value={editNotes[lead.id] ?? lead.notes ?? ""} onChange={(e) => setEditNotes((prev) => ({ ...prev, [lead.id]: e.target.value }))} placeholder="Add notes..." />
                        {(editNotes[lead.id] !== undefined && editNotes[lead.id] !== (lead.notes ?? "")) && <Button size="sm" className="mt-2 font-body" onClick={() => updateLead(lead.id, { notes: editNotes[lead.id] })}>Save Notes</Button>}
                      </div>
                      <div className="flex flex-wrap items-center gap-3">
                        <Popover><PopoverTrigger asChild><Button variant="outline" size="sm" className="font-body text-xs"><CalendarIcon className="w-3 h-3 mr-1" />{lead.follow_up_date ? format(new Date(lead.follow_up_date), "MMM d, yyyy") : "Set Follow-up"}</Button></PopoverTrigger><PopoverContent className="w-auto p-0" align="start"><Calendar mode="single" selected={lead.follow_up_date ? new Date(lead.follow_up_date) : undefined} onSelect={(date) => date && updateLead(lead.id, { follow_up_date: format(date, "yyyy-MM-dd") })} className="p-3 pointer-events-auto" /></PopoverContent></Popover>
                        {lead.status === "new" && <Button size="sm" className="font-body text-xs" onClick={() => updateLead(lead.id, { status: "contacted", last_contacted: new Date().toISOString() })}><CheckCircle className="w-3 h-3 mr-1" /> Mark Contacted</Button>}
                      </div>
                    </div>
                  )}
                </CardContent>
