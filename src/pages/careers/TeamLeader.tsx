...
import {
  ChevronDown, Truck, Shield, Clock, TrendingUp,
  CheckCircle, Upload, Mic, Star,
} from "lucide-react";

const TeamLeaderCareers = () => {
  ...
  const [form, setForm] = useState({
    ...
    comfortable_outdoors: "", why_good_fit: "", best_contact_time: "",
    acknowledged: false,
  });
    ...
    <Layout>
      ...
      <section className="relative bg-foreground text-background overflow-hidden">
        ...
        <div className="container mx-auto section-padding relative z-10 text-center max-w-4xl">
          ...
          <h1 className="font-display text-4xl md:text-6xl font-bold mb-6 text-white">Team Leader</h1>
          <p className="font-body text-lg md:text-xl text-white/70 mb-4 max-w-2xl mx-auto">
            Lead crews in the field for a fast-growing palm tree trimming company that values speed, quality, safety, and professionalism.
          </p>
          ...
          <div className="bg-card border border-border rounded-xl overflow-hidden">
            <div className="bg-foreground p-5">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-display text-xl font-bold text-white">Team Leader Responsibilities</h3>
              </div>
            </div>
            <ul className="p-6 space-y-3">
              {[
                "Drives the truck and trailer to and from job sites",
                "Leads the crew in the field — sets the pace and keeps focus",
                "Talks to the customer on arrival and confirms scope of work",
                "Keeps the job moving efficiently from start to finish",
                "Protects the customer's property at all times",
                "Manages trailer flow, cleanup, and final walkthrough",
                "Handles tools, batteries, chargers, and equipment accountability",
                "Makes sure nothing gets left behind — no fronds, no tools, no mess",
                "Communicates with operations and ownership as needed",
              ].map((item) => (
      ...
      <section className="section-padding bg-background">
        <div className="container mx-auto max-w-4xl">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {[
              { icon: TrendingUp, title: "Competitive Pay", desc: "Hourly pay based on experience and performance. Team Leaders earn more as they prove reliability and leadership." },
              { icon: Star, title: "Room to Grow", desc: "Strong opportunity to grow into a senior leadership role as the company scales." },
              ...
              { icon: Clock, title: "Real Opportunity", desc: "This isn't a dead-end gig. Become a key part of a serious, growing company." },
      ...
      <section className="section-padding bg-secondary/30">
        <div className="container mx-auto max-w-4xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
            {[
              "Leadership ability", "Dependable", "Hard-working", "Strong work ethic",
              "Physically capable", "Detail-oriented", "Works safely",
              "Good communicator", "Situational awareness", "Moves with purpose",
          ...
          <div className="bg-card border border-border rounded-xl p-6">
            <p className="font-body text-muted-foreground">
              Experience in tree work, landscaping, trailers, ladders, saws, cleanup, or outdoor labor goes a long way. But we're still open to the right person if they have the <span className="font-semibold text-foreground">attitude, discipline, and work ethic</span> to learn and lead.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="section-padding bg-secondary/30" ref={formRef} id="apply">
        <div className="container mx-auto max-w-2xl">
          ...
          <form onSubmit={handleSubmit} className="bg-card border border-border rounded-xl p-6 md:p-8 space-y-6">
            ...
            <div className="space-y-2">
              <Label className="font-body font-semibold">Do you have relevant experience? (e.g. tree work, landscaping, trailers, ladders, saws, outdoor labor)</Label>
              <Textarea value={form.has_experience} onChange={(e) => handleChange("has_experience", e.target.value)} placeholder="Tell us about any relevant field experience..." className="font-body" rows={3} />
            </div>
            ...
            <div className="space-y-2">
              <Label className="font-body font-semibold">Are you comfortable working outdoors in heat and doing physical labor?</Label>
              <div className="flex gap-3">
                {["yes", "no"].map((opt) => (
                  <button key={opt} type="button" onClick={() => handleChange("comfortable_outdoors", opt)}
                    className={`font-body text-sm rounded-lg border px-6 py-3 capitalize transition-colors ${form.comfortable_outdoors === opt ? "border-primary bg-primary/10 text-foreground font-semibold" : "border-border bg-background text-muted-foreground hover:border-primary/50"}`}>
                    {opt}
                  </button>
                ))}
              </div>
            ...
            <div className="space-y-2">
              <Label className="font-body font-semibold">Why do you think you'd be a good fit for Team Leader?</Label>
            ...
            <div className="flex items-start gap-3 bg-secondary/50 rounded-lg p-4">
              ...
              <Label htmlFor="acknowledged" className="font-body text-sm text-muted-foreground leading-relaxed cursor-pointer">
                I understand this role requires physical work, reliability, safety awareness, leadership, and strong work ethic.
              </Label>
            </div>
...
export default TeamLeaderCareers;
